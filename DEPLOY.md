# 🚀 Автоматический деплой

## Для Windows

### Первая настройка (один раз)

**Автоматическая настройка (рекомендуется):**

**PowerShell:**
```powershell
.\setup-ssh.bat
```

**CMD:**
```cmd
setup-ssh.bat
```

> 💡 В PowerShell нужно добавлять `.\` перед именем скрипта

Скрипт автоматически создаст SSH ключ и настроит подключение к серверу.

**Или вручную:**

1. **Создать SSH ключ:**
   ```cmd
   ssh-keygen -t rsa -b 4096
   ```
   Нажмите Enter 3 раза

2. **Скопировать ключ на сервер:**
   ```powershell
   type %USERPROFILE%\.ssh\id_rsa.pub | ssh root@89.207.255.13 "cat >> ~/.ssh/authorized_keys"
   ```
   Введите пароль: `r6aQ-osxs0GERy8=`

3. **Проверить:**
   ```cmd
   ssh root@89.207.255.13
   ```
   Должно подключиться без пароля ✅

### Использование

После настройки деплой делается одной командой:

**PowerShell:**
```powershell
.\deploy.bat "описание изменений"
```

**CMD:**
```cmd
deploy.bat "описание изменений"
```

Или быстрый деплой без rebuild:

**PowerShell:**
```powershell
.\deploy-quick.bat "описание изменений"
```

**CMD:**
```cmd
deploy-quick.bat "описание изменений"
```

---

## Для Linux/Mac

### Первая настройка (один раз)

1. **Создать SSH ключ:**
   ```bash
   ssh-keygen -t rsa -b 4096
   ```

2. **Скопировать на сервер:**
   ```bash
   ssh-copy-id root@89.207.255.13
   ```
   Пароль: `r6aQ-osxs0GERy8=`

3. **Проверить:**
   ```bash
   ssh root@89.207.255.13
   ```

### Использование

```bash
make deploy
```

Или быстрый деплой:

```bash
make deploy-quick
```

---

## Что делает деплой?

1. ✅ Добавляет все изменения в git
2. ✅ Создает коммит с вашим сообщением
3. ✅ Пушит в репозиторий
4. ✅ Подключается к серверу
5. ✅ Обновляет код на сервере
6. ✅ Пересобирает Docker образы
7. ✅ Перезапускает контейнеры

**Всё автоматически!** 🎉

---

## Дополнительные команды

### Windows

```cmd
server-logs.bat       # Просмотр логов
server-connect.bat    # Подключение к серверу
```

### Linux/Mac

```bash
make server-logs      # Просмотр логов
make server-status    # Статус контейнеров
make server-shell     # Подключение к серверу
```

---

## Настройка проекта на сервере (первый раз)

Подключитесь к серверу и выполните:

```bash
# Установить Docker
curl -fsSL https://get.docker.com -o get-docker.sh
sh get-docker.sh

# Клонировать проект
cd /root
git clone ВАШ_РЕПОЗИТОРИЙ_URL crm-medical
cd crm-medical

# Настроить окружение
cp .env.prod .env
nano .env  # Отредактировать настройки

# Запустить
docker compose -f docker-compose.yml build
docker compose -f docker-compose.yml up -d
```

---

## Troubleshooting

### Ошибка: Permission denied (publickey)
SSH ключ не настроен. Повторите шаги из раздела "Первая настройка"

### Ошибка: git push failed
```bash
git remote add origin ВАШ_РЕПОЗИТОРИЙ_URL
```

### Контейнеры не запускаются
```bash
ssh root@89.207.255.13
cd /root/crm-medical
docker compose logs
```

---

## 📖 Подробная документация

- **Windows:** [docs/DEPLOY_WINDOWS.md](./docs/DEPLOY_WINDOWS.md)
- **Linux/Mac:** [docs/DEPLOY_SETUP.md](./docs/DEPLOY_SETUP.md)
- **Шпаргалка:** [DEPLOY_CHEATSHEET.md](./DEPLOY_CHEATSHEET.md)

---

## Сервер

- **IP:** 89.207.255.13
- **User:** root
- **Путь:** /root/crm-medical
